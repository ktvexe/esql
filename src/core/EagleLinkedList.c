...
void EagleLinkedList_DeleteItems(EagleLinkedList *list)
{
    ...
    for (EagleLinkedListItem *p = list->first; NULL != p; p = next) {
        next = p->next;
...
void EagleLinkedList_addObject(EagleLinkedList *list, void *obj,
                               EagleBoolean freeObj, void (*free)(void *obj))
{
...
EagleBoolean EagleLinkedList_deleteObject(EagleLinkedList *list, void *obj)
{
    ...
    for (cursor = last->next; cursor;
         cursor = cursor->next, last = last->next) {
        if (obj == cursor->obj) {
