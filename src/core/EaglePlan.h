...
typedef struct EaglePlan {
    /**
     * An operation is a task done on a page. This is semi managed because
     * the actual array that contains the operations will be managed by the
     * object, but the individual operations will be managed externally.
     */
    EAGLE_ATTR_SEMI_MANAGED EagleLinkedList *operations;

    /**
     * The page providers. This is where external data comes into the
     * expression.
     */
    ...
    /**
     * The total time the thread was left waiting for a mutex lock to
     * be released.
     */
 ...
 */
EaglePlanBufferProvider *EaglePlan_getBufferProviderByName(EaglePlan *plan,
                                                           char *name);
 ...
 */
void EaglePlan_setError(EaglePlan *plan, EaglePlanError errorCode,
                        char *errorMessage);
