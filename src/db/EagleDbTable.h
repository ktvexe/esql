...
typedef struct {
    ...
    /**
     * The table columns. This is self managed because the array that contains
     * the pointers to the columns is managed internally, but the columns
     * themselves are managed externally (i.e. deleting the table will not
     * delete the columns)
     */
...
/**
 * Create a new table definition. You will also need to create a
 * EagleDbTableData if you intended the table to hold data.
